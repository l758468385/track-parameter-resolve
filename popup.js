document.addEventListener('DOMContentLoaded', function() {
  ...
  function displayRequests(requests) {
    if (!requests || requests.length === 0) {
      requestsContainer.innerHTML = '<div class="no-requests">暂无捕获到的请求</div>';
      return;
    }
    ...
    div.innerHTML = `
      <div class="request-header">
        <div>
          ...
          ${request.isTargetAPI ? '<span class="target-indicator">目标API</span>' : ''}
        </div>
        <div>
          ${request.decodedData.length > 0 ? `<span class="decoded-count">${request.decodedData.length} 个解码</span>` : ''}
    ...
    if (request.requestData) {
      html += `
        <div style="margin-bottom: 15px;">
          ...
          <div class="original-data">${request.requestData.substring(0, 500)}${request.requestData.length > 500 ? '...' : ''}</div>
        </div>
      `;
    }
    
    // 显示解码的数据
    if (request.decodedData && request.decodedData.length > 0) {
      html += '<div style="margin-bottom: 10px;"><strong>解码的 Base64 数据:</strong></div>';
      
      request.decodedData.forEach((decoded, index) => {
        html += `
          <div class="decoded-item">
            <div class="field-name">字段: ${decoded.field}</div>
            <div style="margin-bottom: 8px;">
              <strong>原始 Base64:</strong>
              <div class="original-data">${decoded.original.substring(0, 200)}${decoded.original.length > 200 ? '...' : ''}</div>
            </div>
            <div>
              <strong>解码结果:</strong>
              <div class="decoded-data">${formatDecodedData(decoded.decoded)}</div>
            </div>
          </div>
        `;
      });
    } else {
      html += '<div style="color: #999; font-style: italic;">此请求未包含 base64 编码数据</div>';
    }
